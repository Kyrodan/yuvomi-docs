...
Continue with [Step 4 — Verify](#verify-the-container-is-running).
...
### 4. Verify the Container is Running {#verify-the-container-is-running}
...
Portainer never places a `.env` file next to the compose file. Whatever you type under **Environment variables** is handed to Compose for `${...}` substitution instead, so the manifest has to list every variable it wants to reach the container. That is exactly what [`docker-compose.portainer.yml`](https://github.com/ulsklyc/yuvomi/blob/main/docs/docker-compose.portainer.yml) does — use it rather than the repository's `docker-compose.yml`.

#### Web editor stack

**Stacks → Add stack → Web editor**, paste the contents of [`docker-compose.portainer.yml`](https://github.com/ulsklyc/yuvomi/blob/main/docs/docker-compose.portainer.yml), then add at least these two under **Environment variables**:
...
> **Self-hosting under the GDPR?** Several optional integrations below (weather, Google/OIDC SSO, WebDAV backup, WebDAV document storage) can send data to third parties, some outside the EU/EEA. See [Privacy for self-hosters](https://github.com/ulsklyc/yuvomi/blob/main/docs/PRIVACY-FOR-SELFHOSTERS.md) for per-service third-country assessments, data-processing-agreement notes and log-retention guidance before enabling them.
...
[Immich screensaver guide](https://github.com/ulsklyc/yuvomi/blob/main/docs/immich-screensaver.md) for setup, Docker networking, preview behavior,
...
| `MODULES_DIR` | Host directory mounted at `/app/modules` inside the container - the drop-in folder for [third-party modules](https://github.com/ulsklyc/yuvomi/blob/main/MODULES.md). Compose-only, like `DATA_DIR`. | `./modules` | No |
...
If the [CHANGELOG](https://github.com/ulsklyc/yuvomi/blob/main/CHANGELOG.md) mentions database migrations or breaking changes, stop the container before updating:
